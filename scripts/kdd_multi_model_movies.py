#!/usr/bin/env python3
"""
KDD 2026: Multi-Model Comparison on Movies Dataset (n=200)
==========================================================
Extends Table 10 (multi-model scaling) from Beauty-only to Movies.
Uses cloud models via Ollama Cloud API + OpenAI.
...
Usage:
    python scripts/kdd_multi_model_movies.py --n_users 200
"""
...
def run_experiment(args):
    print("=" * 70)
    print("KDD 2026: MULTI-MODEL COMPARISON ON MOVIES")
    ...
    if os.getenv("OLLAMA_API_KEY"):
        ...
        print("  [WARN] OLLAMA_API_KEY not set, skipping cloud models")
    ...
    if len(models) == 0:
        print("ERROR: No models available. Set OLLAMA_API_KEY and/or OPENAI_API_KEY.")
        sys.exit(1)
    ...
    # ===== Compute results =====
    print("\n" + "=" * 70)
    print("RESULTS: MULTI-MODEL COMPARISON ON MOVIES")
    print("=" * 70)
    ...
    if p_values:
        ...
        for i, (name, p) in enumerate(comparisons):
            ...
            print(f"  {name:<22} p={p:.4f}  adj_alpha={adj_alpha:.4f}  significant={sig}")
    ...
    # Save
    output_path = project_root / 'experiments' / 'logs' / 'kdd_multi_model_movies.json'

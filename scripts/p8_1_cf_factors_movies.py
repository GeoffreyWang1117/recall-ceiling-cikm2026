"""
Phase 8-1: CF Factor Optimization - Movies Dataset

Test different CF factor sizes to improve recall quality:
- 128 factors (baseline)
- 256 factors
- 512 factors

Goal: Improve Movies recall from 2.8% to 5-10%
"""
...
# ==================== Configuration ====================
CHECKPOINT_PATH = "experiments/logs/checkpoints/p8_1_cf_factors_movies_checkpoint.json"
RESULTS_PATH = "experiments/logs/p8_1_cf_factors_movies.json"
...
def build_prompt_p3_enhanced(
    ...
    """P3: Enhanced CoT with explicit preference signals (Movies version)"""
    ...
    prompt += "  - Identify genres, directors, actors, and themes the user likes\n"
    prompt += "  - Note any patterns in viewing behavior\n"
    prompt += "\nStep 2 - Evaluate Candidates:\n"
    prompt += "  - For each candidate, assess similarity to user's preferred movies\n"
    prompt += "  - Consider movie attributes, genres, and relevance\n"
...
def process_single_user(sample, cf_recalls, llm, dataset):
    """Process a single user with different CF factor sizes"""
...
def main():
    print("=" * 80)
    print("Phase 8-1: CF Factor Optimization - Movies Dataset")
    print("=" * 80)
    ...
    print(f"  Prompt: P3 Enhanced CoT")
    ...
    if len(pending) == 0:
        ...
        print(f"  NOTE: Each user tests {len(FACTORS_TO_TEST)} CF configurations")
    ...
    # ==================== Display Results ====================
    print("\n" + "=" * 80)
    print("Phase 8-1 Results: CF Factor Optimization (Movies)")
    print("=" * 80)
    print()
    print("| CF Factors | Recall@100 | NDCG@10 | Recall@10 | vs 128f |")
    print("|------------|------------|---------|-----------|---------|")

    baseline_ndcg = final_results['cf_128']['ndcg@10']
    baseline_recall = final_results['cf_128']['recall@100']
    ...
    recall_256 = final_results['cf_256']['recall@100']
    recall_512 = final_results['cf_512']['recall@100']

    recall_improvement_256 = ((recall_256 / baseline_recall - 1) * 100) if baseline_recall > 0 else 0
    recall_improvement_512 = ((recall_512 / baseline_recall - 1) * 100) if baseline_recall > 0 else 0

    print(f"  - Baseline (128f): Recall@100={baseline_recall*100:.1f}%, NDCG@10={baseline_ndcg:.4f}")
    print(f"  - 256 factors: Recall@100={recall_256*100:.1f}% ({recall_improvement_256:+.0f}%)")
    print(f"  - 512 factors: Recall@100={recall_512*100:.1f}% ({recall_improvement_512:+.0f}%)")

    if recall_improvement_256 >= 30:
        print(f"  ✅ SUCCESS: 256 factors improve recall by >30%")
    elif recall_improvement_256 >= 10:
        print(f"  ⚠️  PARTIAL: 256 factors improve recall by 10-30%")
    else:
        print(f"  ❌ LIMITED: 256 factors improve recall <10%")
    ...
    output = {
        ...
        'improvements': {
            '256_vs_128': {
                'recall@100': recall_improvement_256,
                'ndcg@10': ((final_results['cf_256']['ndcg@10'] / baseline_ndcg - 1) * 100) if baseline_ndcg > 0 else 0
            },
            '512_vs_128': {
                'recall@100': recall_improvement_512,
                'ndcg@10': ((final_results['cf_512']['ndcg@10'] / baseline_ndcg - 1) * 100) if baseline_ndcg > 0 else 0
            }
        }
    }
    ...
    print("\n" + "=" * 80)
    print("✓ Phase 8-1 (Movies) Complete!")
    print("=" * 80)
